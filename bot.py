...
def send_audio(chat_id, path: Path, filename: str, caption: str,
    ...
    # نرسل لوحة الأزرار مباشرة مع sendAudio.
    ...
    # إذا Telegram لأي سبب ما أظهر الأزرار على رسالة الصوت،
    # نرسل لوحة مستقلة حتى تبقى الوظائف متاحة فعلياً.
    if keyboard is not None:
        has_keyboard = bool(result and result.get("reply_markup"))
        if not has_keyboard:
            send_message(chat_id, "⚙️ تعديل بيانات الملف:", keyboard)
...
def handle_message(message):
    ...
    if text.startswith("/help"):
        send_message(
            ...
            "2) انتظر التضخيم.\n"
            "3) استلم MP3 المضخم.\n"
            "4) استخدم أزرار تعديل الاسم والفنان والصورة والوصف.",
        )
        return
